function createBarChart2(bottomDocData) {
   const container = document.getElementsByClassName("#bottomDocContainer");

   // Check if the container exists
   if (!container) {
       console.error("Error: #bottomDocContainer container not found.");
       return;
   }

   // Prepare dropdown filter options
   const siteNames = Array.from(new Set(bottomDocData.map(d => d.siteName)));
   const dropdown = document.getElementById("site-dropdown");const colorScale = d3.scaleOrdinal(d3.schemeSet3); // Define colorScale using d3.schemeSet3
   ...
   function renderChart(filteredData) {
       ...
       const svg = d3.select("#bottomDocChart")
           .append("svg")
   ...
   dropdown.addEventListener("change", () => {
       ...
       const filteredData = selectedSite === "All" 
           ? bottomDocData 
           : bottomDocData.filter(d => d.siteName === selectedSite);
